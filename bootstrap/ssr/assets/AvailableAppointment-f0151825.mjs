      ...
      if (!__props.appointment.reserved_hours.includes(__props.time)) {
        ...
        _push(ssrRenderComponent(ResuableModal, {
          ...
          content: withCtx((_, _push2, _parent2, _scopeId) => {
            if (_push2) {
              _push2(`<form${_scopeId}><header class="flex items-center justify-center"${_scopeId}><h2 class="text-lg font-bold w-1/3 py-3 rounded-lg flex justify-center items-center bg-blue-500"${_scopeId}>${ssrInterpolate(__props.appointment["full_date"])} ${ssrInterpolate(_ctx.trans("words.at"))} ${ssrInterpolate(__props.time)}</h2></header><section class="flex items-center justify-start my-2"${_scopeId}><label for="location" class="w-36 px-4"${_scopeId}>${ssrInterpolate(_ctx.trans("words.address"))}</label><input type="text" class="block rounded-lg flex-1 px-2 mx-2 h-10" name="location" id="location"${ssrRenderAttr("placeholder", _ctx.trans("words.address"))}${ssrRenderAttr("value", unref(form).location)} required${_scopeId}></section>`);
              if (unref(form).errors.location) {
                _push2(`<p class="error"${_scopeId}>${ssrInterpolate(unref(form).errors.location)}</p>`);
              } else {
                _push2(`<!---->`);
              }
              _push2(`<section class="flex items-center justify-start my-2"${_scopeId}><label for="notes" class="w-36 px-4"${_scopeId}>${ssrInterpolate(_ctx.trans("words.notes"))}</label><textarea class="rounded-lg flex-1 px-2 mx-2 h-40" name="notes" id="notes"${ssrRenderAttr("placeholder", _ctx.trans("words.notes"))}${_scopeId}>${ssrInterpolate(unref(form).notes)}</textarea></section><footer class="flex items-center justify-center"${_scopeId}><button type="submit" class="${ssrRenderClass([{ "opacity-25": unref(form).processing }, "btn btn-primary"])}"${ssrIncludeBooleanAttr(unref(form).processing) ? " disabled" : ""}${_scopeId}>${ssrInterpolate(_ctx.trans("words.confirm"))}</button></footer></form>`);
            } else {
              return [
                createVNode("form", {
                  ...
                  createVNode("header", { class: "flex items-center justify-center" }, [
                    createVNode("h2", { class: "text-lg font-bold w-1/3 py-3 rounded-lg flex justify-center items-center bg-blue-500" }, toDisplayString(__props.appointment["full_date"]) + " " + toDisplayString(_ctx.trans("words.at")) + " " + toDisplayString(__props.time), 1)
                  ]),
